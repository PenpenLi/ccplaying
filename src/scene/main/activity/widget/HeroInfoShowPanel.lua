...
local BUYONEPRICE = 30
local TOTALSOULNUM = 200

function HeroInfoShowPanel:ctor(heroID, num, configID)
    self.data.heroID = heroID
    self.data.currNum = num or 0
...
function HeroInfoShowPanel:createFixedUI()
    ...
    if MODE ~= PRO then
        ...
        self.controls.showHeroID:setPosition(self.data.bgSize.width * 0.77, self.data.bgSize.height * 0.95)
        self.controls.bg:addChild(self.controls.showHeroID)
        self.controls.showHeroID:setVisible(false)

        detailName:setTouchEnable(true)
        detailName:addTouchEventListener(function(sender, eventType)
            if eventType == ccui.TouchEventType.began then
                self.controls.showHeroID:setVisible(true)
                self.controls.showHeroID:setString("星将ID："..self.data.heroID)
            end
...
function HeroInfoShowPanel:heroInfoUI()
    ...
    local desc = Common.finalFont("该将魂限量出售", bgSize.width * 0.25, bgSize.height * 0.26, 20, nil, 1)
    self.controls.bg:addChild(desc)
    desc = Common.finalFont("个", bgSize.width * 0.47, bgSize.height * 0.26, 20, nil, 1)
    self.controls.bg:addChild(desc)
    self.controls.soulNum =  Common.finalFont("", bgSize.width * 0.4, bgSize.height * 0.26, 20, cc.c3b(255, 255, 0), 1)
    self.controls.bg:addChild(self.controls.soulNum)

    self.controls.btn_buyOne = createMixScale9Sprite("image/ui/img/btn/btn_818.png", "image/ui/img/btn/btn_819.png", nil, cc.size(140, 56))
    self.controls.btn_buyOne:setButtonBounce(false)
    self.controls.btn_buyOne:setFont("购买1个" , 1, 1, 25, cc.c3b(248, 216, 136))
    self.controls.btn_buyOne:setFontOutline(cc.c4b(70, 50, 14, 255), 1)
    self.controls.btn_buyOne:setPosition(bgSize.width * 0.23, bgSize.height * 0.16)
    self.controls.bg:addChild(self.controls.btn_buyOne)
    self.controls.btn_buyOne:addTouchEventListener(function(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            self:buyHeroSoul(1)
        end
    end)

    self.controls.btn_buyTen = createMixScale9Sprite("image/ui/img/btn/btn_818.png", "image/ui/img/btn/btn_819.png", nil, cc.size(140, 56))
    self.controls.btn_buyTen:setButtonBounce(false)
    self.controls.btn_buyTen:setFont("购买10个" , 1, 1, 25, cc.c3b(248, 216, 136))
    self.controls.btn_buyTen:setFontOutline(cc.c4b(70, 50, 14, 255), 1)
    self.controls.btn_buyTen:setPosition(bgSize.width * 0.44, bgSize.height * 0.16)
    self.controls.bg:addChild(self.controls.btn_buyTen)
    self.controls.btn_buyTen:addTouchEventListener(function(sender, eventType)
        if eventType == ccui.TouchEventType.ended then
            self:buyHeroSoul(10)
        end
    end)
    ...
    goldSpri:setPosition(bgSize.width * 0.19, bgSize.height * 0.09)
    self.controls.bg:addChild(goldSpri)
    goldSpri = cc.Sprite:create("image/ui/img/btn/btn_060.png")
    goldSpri:setPosition(bgSize.width * 0.4, bgSize.height * 0.09)
    self.controls.bg:addChild(goldSpri)
    local lab_price = Common.finalFont(BUYONEPRICE, bgSize.width * 0.24, bgSize.height * 0.09, 20, cc.c3b(255, 255, 0), 1)
    self.controls.bg:addChild(lab_price)
    lab_price = Common.finalFont(BUYONEPRICE * 10, bgSize.width * 0.45, bgSize.height * 0.09, 20, cc.c3b(255, 255, 0), 1)
    self.controls.bg:addChild(lab_price)
end
...
function HeroInfoShowPanel:updateInfo()
    self.controls.soulNum:setString(self.data.currNum.."/"..TOTALSOULNUM)
end

function HeroInfoShowPanel:buyHeroSoul(num)
    if Common.isCostMoney(1001, num * BUYONEPRICE) then
        if (self.data.currNum + num) > TOTALSOULNUM then
            application:showFlashNotice("已超出购买个数～")
            return 
        end
        rpc:call("Activity.BuyHeroRecommand", {Id = self.data.configID, Num = num}, function(event)
            if event.status == Exceptions.Nil then
                self.data.currNum = self.data.currNum + num
                self:updateInfo()
                ...
                GameCache.addSoul(heroSoulInfo)

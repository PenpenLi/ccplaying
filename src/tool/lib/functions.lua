...
function handler(target, method, ...)
    assert(target, "handler(target == nil)")
    assert(method, "handler(method == nil)")
    
    local bindArgs = {...}
    return function(...)
        if #bindArgs > 0 then
            return method(target, unpack(bindArgs), ...)
        else
...
function io.exists(path)
    ...
    CCLog(vardump({filePath = filePath, path = path}, "io.exists"))

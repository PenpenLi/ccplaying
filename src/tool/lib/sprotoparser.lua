...
local function extract( n, i, j )
	local hex = bit.tohex(n)
	local sub = "0x" .. string.sub(hex, i, j)
...
local function packbytes( str )
	local size = #str
	return string.char(extract(size,7,8))..
		string.char(extract(size,5,6))..
		string.char(extract(size,3,4))..
		string.char(extract(size,1,2))..
		str
end
...
local function packvalue(id)
	id = (id + 1) * 2
	assert(id >=0 and id < 65536)
	return string.char(extract(id, 7, 8)) .. string.char(extract(id, 5, 6))
end

...
local function percolate_up(self,index)
  ...
  if index > 1 then
    ...
    if self._heap[pIndex] then
      if not (self.sort(self._heap[pIndex],self._heap[index])) then
...
local function percolate_down(self,index)
  ...
  if rtIndex > self.size then
    if lfIndex > self.size then 
        return
    else 
        minIndex = lfIndex  
    end
  else
    if self.sort(self._heap[lfIndex],self._heap[rtIndex]) then
      minIndex = lfIndex
    else
      minIndex = rtIndex
    end
  end
  if not self.sort(self._heap[index],self._heap[minIndex]) then
  ...
  for i = 1,(floor(self.size/2)) do
    ...
    rtIndex = lfIndex+1
      if self:checkIndex(lfIndex) then
        if not self.sort(self._heap[i],self._heap[lfIndex]) then
          return false
        end
      end
      if self:checkIndex(rtIndex) then
        if not self.sort(self._heap[i],self._heap[rtIndex]) then
          return false
        end
      end
  end
...
function heap.__tostring(self)
  local item = table.concat(self._heap, ", ")
  -- for k in ipairs(self._heap) do
  --   out = out.. (('Element %d - Value : %s\n'):format(k,tostring(self._heap[k])))
  -- end
  return string.format("heap{size = %d, item = {%s}}", self.size, item)
end
